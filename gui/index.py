"""module contains the IndexUI class and functions to support it"""
...
def get_selected_index(tree):
    """function returns the index of the selected row in the tree view"""
    item = tree.focus()
    if item:
...
class IndexUI:
    """IndexUI class contains all of the methods and UI elements for the View All Bookings page"""

    def __init__(self, master):
        """when init called the UI elements for view all will be displayed on the master given"""
        # variables
        ...
        self.futureDateCheckBox = Checkbutton(self.master, text="Future Dates only", bg=style.widgetBG,
        ...
        self.buttonBack = Button(self.buttonFrame, text='Back', font=style.textNormal, bg="snow", height=2, width=10,
                                 command=self.back_to_main_menu)
        self.buttonViewDetails = Button(self.buttonFrame, text='View Details', font=style.textNormal, state=DISABLED,
                                        bg=style.buttonColour1,
                                        height=2, width=10, command=lambda: self.open_view_details())
        self.buttonEdit = Button(self.buttonFrame, text='Edit', font=style.textNormal, state=DISABLED,
                                 bg=style.buttonColour2, height=2, width=10, command=self.open_edit)
        ...
        self.tree.bind('<ButtonRelease-1>', self.activate_buttons)
        ...
        self.refresh_eventlist()  # displaying loaded bookings from database in tree view

    def activate_buttons(self, event):
        """will activate option buttons when item selected in tree view"""
    ...
    def deactivate_buttons(self):
        """will deactivate option buttons"""
    ...
    def refresh_eventlist(self):
        """refresh the bookings displayed in the tree view"""
        self.TotalLabel['text'] = len(self.events)
    ...
    def getdata(self, eventType='All Types', future=False):
        """returns a list of bookings given the event type searched and if the date of event is in the future"""
        future = future
    ...
    def updateEvents(self, event):
        """refresh the list of bookings from the database and updates the display in the tree view to show these"""
    ...
    def refresh_onfocus(self, event):
        """method ran when window gains focus to update bookings being shown in tree view"""
        if self.focusOut:
            self.updateEvents(None)
            self.focusOut = False
            print('focus')

    def open_view_details(self):
        """opens a top level of the view details UI for the a selected booking in the tree view"""
        view_details_popup(self.events[get_selected_index(self.tree)], self.master)

    def open_edit(self):
        """opens a top level of the update UI for the a selected booking in the tree view"""
        self.focusOut = True
